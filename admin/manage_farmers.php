...
// Pull all farmer rows into an array once, so we can loop over them
// twice (once for the table, once for the modals) without re-querying.
$farmers = [];
while ($row = $farmersResult->fetch_assoc()) {
    $farmers[] = $row;
}
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($farmers as $f): ?>
                        <?php $crops = $cropsByFarmer[$f['farmer_id']] ?? []; ?>
                        <tr>
                            <td><?php echo htmlspecialchars($f['full_name']); ?></td>
                            <td><?php echo htmlspecialchars($f['location']); ?></td>
                            <td>
                                <?php echo $crops ? htmlspecialchars(implode(', ', $crops)) : '<span class="text-muted">None</span>'; ?>
                            </td>
                            <td>
